...
exports.deleteInventory = asyncHandler(async (req, res) => {
    ...
    if (!inventory.isActive) {
        throw new ApiError(400, "Inventory item is already inactive");
    }
    ...
    res
        .status(200)
        .json(new ApiResponse(200, inventory, "Inventory item deactivated successfully"));
});
...
exports.getWeatherAdvisory = asyncHandler(async (req, res) => {
    ...
    if (temp > 35) {
        advisories.push({
            level: "WARNING",
            message:
                "High temperature detected. Ensure perishables are stored in cool areas. Leafy vegetables may wilt faster.",
    ...
    if (humidity > 80) {
        advisories.push({
            level: "CAUTION",
            message:
                "High humidity detected. Risk of mould and fungal growth. Check stored grains and dried produce.",
    ...
    if (weatherMain.includes("rain") || weatherMain.includes("storm")) {
        advisories.push({
            level: "INFO",
            message:
                "Rainfall expected. Good conditions for leafy crops but protect harvested produce from moisture.",
