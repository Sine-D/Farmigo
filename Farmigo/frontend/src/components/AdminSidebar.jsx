...
const AdminSidebar = () => {
  ...
  const navItems = [
    ...
    // support-related
    // { icon: <FaTicketAlt />, label: 'Tickets', path: '/support?tab=tickets' },
    // { icon: <FaGavel />, label: 'Disputes', path: '/support?tab=disputes' },
  ...
  const isActivePath = (itemPath) => {
    if (itemPath === '/support?tab=tickets') {
      return (
    ...
    if (itemPath === '/support?tab=disputes') {
      return (
        location.pathname === '/support' &&
        new URLSearchParams(location.search).get('tab') === 'disputes'
      );
    }
